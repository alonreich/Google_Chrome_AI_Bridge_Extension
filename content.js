    ...
    if (window === window.top) {
        ...
        function ensureEyeStyles() {
            ...
            style.textContent = `
                ...
                /* 8-step sine curve: 0% and 100% identical for seamless loop (no end jump) */
                ...
                #${OVERLAY_ID} .bridge-overlay-eye {
                    width: min(72vmin, 72vh);
                    height: min(72vmin, 72vh);
        ...
        function resolveEyeMode(request) {
            if (request.eyeMode === 'busy' || request.eyeMode === 'idle') return request.eyeMode;
            return request.isBusy ? 'busy' : 'idle';
        }

        if (!window.geminiBridgeHeartbeatStarted) {
            window.geminiBridgeHeartbeatStarted = true;
            setInterval(() => {
                chrome.runtime.sendMessage({ action: 'bridge_heartbeat' }).catch(() => {});
            }, 20000);
        }

        if (!window.geminiBridgeInitialized) {
            window.geminiBridgeInitialized = true;

            window.geminiCreateEyes = function (mode) {
            ...
            window.geminiShowLargeOverlay = function (mode) {
                const eyeMode = mode === 'busy' ? 'busy' : 'idle';
            ...
            window.geminiRemoveEyes = function () {
                document.querySelectorAll('#' + EYES_ID).forEach((el) => el.remove());
                document.querySelectorAll('#' + OVERLAY_ID).forEach((el) => el.remove());
            };

            chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
                if (request.action === 'update_eyes') {
                    if (request.active) {
                        window.geminiCreateEyes(resolveEyeMode(request));
                    } else {
                        window.geminiRemoveEyes();
                    }
                    sendResponse({ status: 'updated' });
                } else if (request.action === 'show_overlay') {
                    window.geminiShowLargeOverlay('idle');
                    sendResponse({ status: 'overlay' });
                } else if (request.action === 'ping') {
                    sendResponse({ status: 'alive', eyeMode: 'idle' });
                } else {
        ...
        chrome.runtime.sendMessage({ action: 'get_bridge_state' }, (response) => {
            if (chrome.runtime.lastError) {
                window.geminiRemoveEyes();
                return;
            }
            if (response && response.active) {
                window.geminiCreateEyes(response.isBusy ? 'busy' : 'idle');
            } else {
                window.geminiRemoveEyes();
            }
        });
